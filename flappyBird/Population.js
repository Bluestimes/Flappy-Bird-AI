class Population {
    ...
    naturalSelection () {
        ...
        for (let j = 0; j < this.species.length; j++) { // for each this.species
            ...
            let NoOfChildren = floor(this.species[j].averageFitness / averageSum * this.players.length) - 1; // the number of children this this.species is allowed, note -1 is because the champ is already added
    ...
    speciate () {
        ...
        for (let i = 0; i < this.players.length; i++) { // for each player
            ...
            for (let s of this.species) { // for each this.species
    ...
    playerInBatch (player) {
        for (let i = this.batchNo * this.worldsPerBatch; i < min((this.batchNo + 1) * this.worldsPerBatch, worlds.length); i++) {
    ...
    batchDead () {
        for (let i = this.batchNo * this.playersPerBatch; i < min((this.batchNo + 1) * this.playersPerBatch, this.players.length); i++) {
            if (!this.players[i].dead) {
                return false;
            }
        }
